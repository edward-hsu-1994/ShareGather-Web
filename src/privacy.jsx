...
const policies = {
  'zh-Hant': { title:'隱私權政策', back:'← 回到 ShareGather', updated:'最後更新：2026 年 7 月 24 日', intro:'ShareGather 旨在將你想日後回看的內容保存在 iPhone 本機。本政策說明 App 如何處理這些內容。', sections:[['ShareGather 儲存的資訊','當你透過 iOS 分享選單或「Save to ShareGather」動作分享內容時，App 可能儲存你選擇分享的網址和文字；以及網址可用時的標題、描述、來源與縮圖。分類、排序、釘選設定和語言偏好也會儲存在裝置上。這些資料只用於提供保存、整理、預覽與再次分享功能。'],['沒有帳號、追蹤或雲端服務','ShareGather 不需要帳號或登入，也沒有自營伺服器或雲端儲存服務。App 不包含分析、廣告、追蹤或資料經紀整合，也不會將你的收藏傳送至 ShareGather 控制的伺服器。'],['網址預覽資料請求','對於分享的網址，ShareGather 可能使用 Apple 的 Link Presentation 請求標題或縮圖等預覽資料。這是可選的；即使預覽載入失敗，原始網址仍會被保存。網址的網站及其服務可能會收到你的網路資訊，並依其隱私政策處理該請求。'],['備份與還原','當你選擇透過系統分享選單匯出備份時，備份檔可能包含收藏項目、分類與網址縮圖。接收備份的 App 或服務將依其自身隱私政策處理該檔案。匯入時，ShareGather 會讀取你選擇的檔案並儲存在本機。'],['你的選擇與資料刪除','你可以刪除個別項目、分類，或使用「Clear All Saved Items」清除已儲存內容。清除後會移除項目資料及關聯的本機網址縮圖。移除 App 也可能移除本機資料；如需保留資料，請先匯出備份。'],['聯絡我們','若有隱私問題或意見，歡迎在 ShareGather GitHub 儲存庫提出 issue。']] },
  'zh-Hans': { title:'隐私政策', back:'← 返回 ShareGather', updated:'最后更新：2026 年 7 月 24 日', intro:'ShareGather 用于将你想日后回看的内容保存在 iPhone 本机。本政策说明 App 如何处理这些内容。', sections:[['ShareGather 储存的信息','当你通过 iOS 分享菜单或“Save to ShareGather”动作分享内容时，App 可能储存你选择分享的网址和文字；以及网址可用时的标题、描述、来源与缩略图。分类、排序、置顶设置和语言偏好也会储存在设备上。这些数据只用于提供保存、整理、预览与再次分享功能。'],['没有账号、追踪或云端服务','ShareGather 不需要账号或登录，也没有自营服务器或云端储存服务。App 不包含分析、广告、追踪或数据经纪整合，也不会将你的收藏发送至 ShareGather 控制的服务器。'],['网址预览数据请求','对于分享的网址，ShareGather 可能使用 Apple 的 Link Presentation 请求标题或缩略图等预览数据。这是可选的；即使预览加载失败，原始网址仍会被保存。网址的网站及其服务可能会收到你的网络信息，并依其隐私政策处理该请求。'],['备份与恢复','当你选择通过系统分享菜单导出备份时，备份文件可能包含收藏项目、分类与网址缩略图。接收备份的 App 或服务将依其自身隐私政策处理该文件。导入时，ShareGather 会读取你选择的文件并储存在本机。'],['你的选择与数据删除','你可以删除单个项目、分类，或使用“Clear All Saved Items”清除已保存内容。清除后会移除项目数据及关联的本机网址缩略图。移除 App 也可能移除本机数据；如需保留数据，请先导出备份。'],['联系我们','如有隐私问题或意见，欢迎在 ShareGather GitHub 仓库提出 issue。']] },
